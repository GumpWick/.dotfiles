...
static void quit_properly(const Arg *arg);	// ~~~~~~~~~~~~~~ HERE ~~~~~~~~~~~~
static void smart_kill_with_ctrlq_firefox(const Arg *arg);   // ~~~~~~~~~~~~~~ HERE ~~~~~~~~~~~~
...
drawbar(Monitor *m)
{
	...
	if (m == selmon || 1) { /* status is only drawn on selected monitor */
		char *text, *s, ch;
		drw_setscheme(drw, scheme[SchemeStatus]); //SchemeNorm AVANT
	...
	w = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeTagsNorm]); //SchemeNorm AVANT
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0);

	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeInfoSel : SchemeInfoNorm]); //modifiÃ aussi
...
//
//
//     /* centre si la fenêtre est flottante ou en layout flottant et non positionnée */
...
manage(Window w, XWindowAttributes *wa)
{
    ...
    /* détermine si la fenêtre doit être flottante */
    if (!c->isfloating)
        c->isfloating = c->oldstate = trans != None || c->isfixed;
    ...
    /* évite que la fenêtre sorte de l'écran */
    if (c->x + WIDTH(c) > c->mon->wx + c->mon->ww)
        c->x = c->mon->wx + c->mon->ww - WIDTH(c);
    if (c->y + HEIGHT(c) > c->mon->wy + c->mon->wh)
        c->y = c->mon->wy + c->mon->wh - HEIGHT(c);
...
char *get_window_class(Window win) {
    ...
    if (XGetClassHint(dpy, win, &class_hint)) {
        char *res = strdup(class_hint.res_class);
...
void smart_kill_with_ctrlq_firefox(const Arg *arg) {
    if (!selmon->sel)
        return;

    Window win = selmon->sel->win;
    char *class = get_window_class(win);

    if (class && strcmp(class, "Firefox") == 0) {
        // Envoie Ctrl+Q
        KeyCode ctrl = XKeysymToKeycode(dpy, XK_Control_L);
        KeyCode q = XKeysymToKeycode(dpy, XK_q);

        XTestFakeKeyEvent(dpy, ctrl, True, 0);  // Press Ctrl
        XTestFakeKeyEvent(dpy, q, True, 0);     // Press Q
        XTestFakeKeyEvent(dpy, q, False, 0);    // Release Q
        XTestFakeKeyEvent(dpy, ctrl, False, 0); // Release Ctrl
        XFlush(dpy);
    } else {
        killclient(arg);
    }
...
resizeclient(Client *c, int x, int y, int w, int h)
{
  ...
  //// TEST perso no borders qd une seule fenêtre d'ouverte
...
restack(Monitor *m)
{
	...
	if (m == selmon && (m->tagset[m->seltags] & m->sel->tags) && m->lt[m->sellt]->arrange != &monocle
			&& !strstr(m->sel->name, "xfce4-notifyd"))
		warp(m->sel);
	XSync(dpy, False);
	while (XCheckMaskEvent(dpy, EnterWindowMask, &ev));
}
